...
def mf1(
    ...
    if return_scores:
        return float(np.mean(f1_macro_scores)), f1_macro_scores
    return float(np.mean(f1_macro_scores))
...
def uar(
    ...
    if return_scores:
        return float(np.mean(uar_scores)), uar_scores
    return float(np.mean(uar_scores))


def uar_ah(y_true, y_pred) -> float:
    """UAR (macro recall) for single-label classification."""
    return float(recall_score(y_true, y_pred, average="macro", zero_division=0))


def mf1_ah(y_true, y_pred) -> float:
    """Macro F1 for single-label classification."""
    return float(f1_score(y_true, y_pred, average="macro", zero_division=0))


def acc_func(trues, preds) -> float:
    acc = []
    for i in range(5):
